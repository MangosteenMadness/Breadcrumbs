...
| 2 | spec-author | The committed `ingestion/cairn.db` cannot be migrated by editing `graph_schema.sql`: `connect()` runs `executescript` with `CREATE TABLE IF NOT EXISTS`, so the old CHECK constraints survive. A table rebuild is required, and `finding_edges` cascades on delete from `findings` — a rebuild with foreign keys enabled would silently destroy every edge. Back the DB up before the first run. | warn | open |
