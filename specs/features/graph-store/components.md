# Breadcrumbs / Graph Store — Component Registry

> The ordered list of components. The same IDs, in the same order, must appear in `spec.tech.md`
> and `feature.json.component_ids`.
> Status vocabulary: built-at-parity | gap | not-built | descoped | unverified | planned | in_progress

## GRAPH — findings, edges, and provenance

| ID | Component | Status |
|----|-----------|--------|
| BC-GRAPH-001 | Findings node table; status vocabulary gains `open` | built-at-parity |
| BC-GRAPH-002 | Typed finding edges — `duplicate_of` / `extends` / `related` / `contradicts` | built-at-parity |
| BC-GRAPH-003 | External literature store — host agent owns literature research | descoped |
| BC-GRAPH-004 | K Pro chat provenance tables and ingestion | built-at-parity |
| BC-GRAPH-005 | Reviewed-write path — validation as the human-confirm gate | built-at-parity |
| BC-GRAPH-006 | Schema migration — CHECK-constraint table rebuild | built-at-parity |
| BC-GRAPH-007 | Dataset catalog — K Pro Explore Data table/column metadata | gap |
| BC-GRAPH-008 | Approved interaction knowledge — source-linked patches with FTS5 and model-versioned dense index | built-at-parity |
| BC-GRAPH-009 | Canonical people, identity evidence, contribution edges, and confirmed investigation edges | built-at-parity |
